...
The code tutorial [p2p programming in github.com/go-ethereum-analysis](https://github.com/nolash/github.com/go-ethereum-analysis-p2p-demo) by [@nolash](https://github.com/nolash) provides step-by-step code examples for usage of `pss` API with `github.com/go-ethereum-analysis` nodes.
...
For `golang` clients, please use the `rpc.Client` provided by the `github.com/go-ethereum-analysis` repository. The return values may have special types in `golang`. Please refer to `godoc` for details.
