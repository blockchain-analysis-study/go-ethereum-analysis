...
/**
让我们假设为每个 req 提供服务都会给服务器带来一定的成本（取决于类型和参数）。
该费用由服务器确定，但对于任何有效请求都有上限。
服务器为每个客户端分配一个“缓冲区”，从中可以推断出每个请求的成本。
缓冲区具有上限（“缓冲区限制”）和充电速率（每秒成本）。
如果服务器有更多的可用资源，则可以决定在任何时候更快地为其充电，但是可以保证最低的充电率。
如果收到的请求会将客户端的缓冲区消耗到零以下，则表明客户端违反了流控制规则，并受到限制或断开连接。
 */
